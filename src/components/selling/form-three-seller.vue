    ...
    <div>
        ...
        <div id="form3">
            <div class="formbox">
                ...
                <input type="radio" id="VAT-option1" name="VAT" class="VAT" value="VAT-No" v-model="VAT" @click="showVAToffice(false)" />
                <label for="VAT-option1" class="section">ไม่</label>
                <input type="radio" id="VAT-option2" name="VAT" class="VAT" value="VAT-Yes" v-model="VAT" @click="showVAToffice(true)" />
                ...
                <div id="VAT-office" ref="VATOffice" v-show="isVATOfficeVisible">
                    ...
                    <input type="radio" id="office" name="office" value="office" v-model="office" @click="handleInput()" />
                    ...
                    <div class="formbox">
                        ...
                        <div class="upload-container">
                            ...
                            <font-awesome-icon v-if="VATImages.length === 0" :icon="['fas', 'image']" @click="triggerFileInput('VAT-certificate')" class="upload-icon" />
                            <input type="file" name="image" ref="VATInput" class="image-upload" accept="image/*" @change="previewImage($event, 'VATImages')" />
...
export default {
    data() {
        return {
            ...
            prefix: sellerData.prefix,
            otherPrefix: sellerData.otherPrefix,
            firstname: sellerData.firstname,
            lastname: sellerData.lastname,
            idCardNumber: sellerData.idCardNumber,
            birthday: sellerData.birthday,
            province: sellerData.province,
            amphoe: sellerData.amphoe,
            district: sellerData.district,
            zipcode: sellerData.zipcode,
            addressDetails: sellerData.addressDetails,
            idCardImages: sellerData.idCardImages,
            pairIdCardImages: sellerData.pairIdCardImages,

            corporateType: sellerData.corporateType,
            corporateName: sellerData.corporateName,
            corporateRegistrationNumber: sellerData.corporateRegistrationNumber,
            provinceCorporate: sellerData.provinceCorporate,
            amphoeCorporate: sellerData.amphoeCorporate,
            districtCorporate: sellerData.districtCorporate,
            zipcodeCorporate: sellerData.zipcodeCorporate,
            detailsCorporate: sellerData.detailsCorporate,
            companyCertificateImages: sellerData.companyCertificateImages,
            directorIdCardImages: sellerData.directorIdCardImages
        };
    },
    methods: {
        validateForm3() {
        ...
        redirectToSellingPage() {
            if(this.validateForm3()) {
                this.saveData();
                this.currentForm = 4;
                this.$router.push("/users/FormFourSeller");
            } else {
                this.form3Error = true;
            }
        },
        saveData() {
            ...
            axios.post('http://localhost:8081/save-data', data)
                .then(response => {
        ...
        redirectToFormTwoSeller() {
            this.$router.push("/form-two-seller"); // Assuming '/selling' is the route path for selling.ejs
        },
    },
};
</script>
