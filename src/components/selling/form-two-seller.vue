        ...
        <div id="form2">
            ...
            <div id="individual-container" v-show="sellerType === 'individual'">
                ...
                <div class="formbox">
                    ...
                    <input type="text" name="firstname" ref="firstname" id="firstname" class="input-form" placeholder="First Name"
                        v-model="firstname" @input="handleInput()"/>
                </div>
                <div class="formbox">
                    <label class="section">นามสกุล :</label>
                    <input type="text" name="lastname" ref="lastname" id="lastname" class="input-form" placeholder="Last Name"
                        v-model="lastname" @input="handleInput()"/>
                </div>
                <div class="formbox">
                    <label class="section">หมายเลขบัตรประชาชน :</label>
                    <input type="text" name="id-card-number" id="id-card-number" class="input-form"
                        placeholder="ID Card Number" minlength="13" ref="idCardNumber" maxlength="13" v-model="idCardNumber "
                ...
                <div class="formbox">
                    ...
                    <input type="date" name="birthday" ref="birthday" id="birthday" class="input-form" placeholder="Birthday"
                        v-model="birthday" @input="handleInput()"/>
                ...
                <div class="formbox">
                    ...
                    <div class="formbox">
                        ...
                        <input type="text" id="province" ref="province" name="province" class="input-form" placeholder="จังหวัด"
                            v-model="province" @input="handleInput()" />
                    </div>
                    <div class="formbox">
                    ...
                    <div class="formbox">
                        ...
                        <input type="text" id="district" ref="district" name="district" class="input-form" placeholder="ตำบล"
                            v-model="district" @input="handleInput()"/>
                    </div>
                    <div class="formbox">
                        <label class="section">รหัสไปรษณีย์ :</label>
                        <input type="text" id="zipcode" ref="zipcode" name="zipcode" class="input-form" placeholder="รหัสไปรษณีย์"
                            v-model="zipcode" @input="handleInput()"/>
                    </div>
                    <div class="formbox">
                        <textarea name="address-details" ref="addressDetails" id="address-details" class="input-form"
                            placeholder="รายละเอียดที่อยู่ ระบุตามที่อยู่ตามบัตรประชาชน"
                            v-model="addressDetails" @input="handleInput()"></textarea>
                    </div>
                </div>
                <div class="formbox">
                    ...
                    <div class="upload-container">
                        ...
                        <font-awesome-icon v-if="idCardImages.length === 0" :icon="['fas', 'image']" @click="triggerFileInput('photo-IDcard')" class="upload-icon" />
                        <input type="file" name="image" ref="photoIDcardInput" class="image-upload" accept="image/*" @change="previewImage($event, 'idCardImages')" />
                ...
                <div class="formbox">
                    ...
                    <div class="upload-container">
                        ...
                        <font-awesome-icon v-if="pairIdCardImages.length === 0" :icon="['fas', 'image']" @click="triggerFileInput('photo-pair-IDCard')" class="upload-icon" />
                        <input type="file" name="image" ref="photoPairIDcardInput" class="image-upload" accept="image/*" @change="previewImage($event, 'pairIdCardImages')" />
                    </div>
            ...
            <div id="corporate-container" v-show="sellerType === 'corporate'">
                <div class="formbox">
                    ...
                    <input type="radio" id="corporate-option1" class="corporateType" v-model="corporateType" value="Limited" @click="handleInput()"/>
                    <label for="corporate-option1" class="section" >บริษัทจำกัด</label>
                    <input type="radio" id="corporate-option2" class="corporateType" v-model="corporateType" value="PublicLimited" @click="handleInput()"/>
                    <label for="corporate-option2" class="section">บริษัทจำกัดมหาชน</label>
                    <input type="radio" id="corporate-option3" class="corporateType" v-model="corporateType" value="Partnership" @click="handleInput()"/>
                    <label for="corporate-option3" class="section">ห้างหุ้นส่วนสามัญนิติบุคคล</label>
                    <input type="radio" id="corporate-option4" class="corporateType" v-model="corporateType" value="LimitedPartnership" @click="handleInput()"/>
                    <label for="corporate-option4" class="section">ห้างหุ้นส่วนจำกัด</label>
                    <input type="radio" id="corporate-option5" class="corporateType" v-model="corporateType" value="Foundation" @click="handleInput()"/>
                    <label for="corporate-option5" class="section">มูลนิธิ</label>
                    <input type="radio" id="corporate-option6" class="corporateType" v-model="corporateType" value="Association" @click="handleInput()"/>
                ...
                <div class="formbox">
                    ...
                    <input type="text" ref="corporateName" v-model="corporateName" class="input-form" placeholder="Corporate Name" @input="handleInput()"/>
                </div>
                <div class="formbox">
                    <label class="section">เลขจดทะเบียนนิติบุคคล :</label>
                    <input type="text" ref="corporateRegistrationNumber" v-model="corporateRegistrationNumber" class="input-form"
                        placeholder="Corporate Registration Number" minlength="13" maxlength="13"
                ...
                <div class="formbox">
                    ...
                    <div class="formbox">
                        <label class="section">จังหวัด :</label>
                        <input type="text" id="provinceCorporate" name="provinceCorporate" ref="provinceCorporate" v-model="provinceCorporate" class="input-form" placeholder="จังหวัด" @input="handleInput()"/>
                    </div>
                    <div class="formbox">
                        <label class="section">อำเภอ :</label>
                        <input type="text" id="amphoeCorporate" name="amphoeCorporate" ref="amphoeCorporate" v-model="amphoeCorporate" class="input-form" placeholder="อำเภอ" @input="handleInput()"/>
                    </div>
                    <div class="formbox">
                        <label class="section">ตำบล :</label>
                        <input type="text" id="districtCorporate" name="districtCorporate" ref="districtCorporate" v-model="districtCorporate" class="input-form" placeholder="ตำบล" @input="handleInput()"/>
                    </div>
                    <div class="formbox">
                        <label class="section">รหัสไปรษณีย์ :</label>
                        <input type="text" id="zipcodeCorporate" name="zipcodeCorporate" ref="zipcodeCorporate" v-model="zipcodeCorporate" class="input-form" placeholder="รหัสไปรษณีย์" @input="handleInput()"/>
                    </div>
                    <div class="formbox">
                        <textarea name="details-corporate" ref="detailsCorporate" id="details-corporate" v-model="detailsCorporate" class="input-form"
                            placeholder="ที่อยู่จดทะเบียนบริษัท กรอกที่อยู่ตามหนังสือรับรอง/ภ.พ.20 ฉบับล่าสุด" @input="handleInput()"></textarea>
                    </div>
                </div>
                <div class="formbox">
                    ...
                    <div class="upload-container">
                        <div class="images-container" id="container-company-certificate">
                            <div v-for="(image, index) in companyCertificateImages" :key="index" class="image-wrapper">
                                ...
                                <span class="delete-icon" @click="removeImage(index, 'companyCertificateImages')">x</span>
                            </div>
                        </div>
                        <font-awesome-icon v-if="companyCertificateImages.length === 0" :icon="['fas', 'image']" @click="triggerFileInput('company-certificate')" class="upload-icon" />
                        <input type="file" name="image" ref="companyCertificateInput" class="image-upload" accept="image/*" @change="previewImage($event, 'companyCertificateImages')" />
                ...
                <div class="formbox">
                    ...
                    <div class="upload-container">
                        ...
                        <font-awesome-icon v-if="directorIdCardImages.length === 0" :icon="['fas', 'image']" @click="triggerFileInput('director-id-card')" class="upload-icon" />
                        <input type="file" name="image" ref="directorIdCardInput" class="image-upload" accept="image/*" @change="previewImage($event, 'directorIdCardImages')" />
...
export default {
    ...
    methods: {
        ...
        redirectToFormOneSeller() {
            this.$router.push("/form-one-seller"); // Assuming '/selling' is the route path for selling.ejs
        },
        redirectToFormThreeSeller() {
            if (this.validateForm2()) {
                this.saveData();
                this.$router.push("/users/FormThreeSeller");
            } else {
                this.form2Error = true;
            }
        },
        ...
        saveData() {
            if (this.sellerType === "individual") {
                ...
                axios.post('http://localhost:8081/save-data', data1)
                .then(response => {
            ...
            if (this.sellerType === "corporate") {
                ...
                axios.post('http://localhost:8081/save-data', data2)
                .then(response => {
                    console.log(response.data);
                })
                .catch(error => {
