<template>
    <div>
        ...
        <div id="form1"  v-show="currentForm === 1">
            <div class="formbox">
                <label class="section">ชื่อร้านค้า :</label>
                <input type="text" id="shop-name" name="shop-name" class="input-form" placeholder="Name"
                    v-model="user.name" />
            </div>
            <div class="formbox">
            ...
            <div class="formbox">
                ...
                <input type="tel" id="phone-number" name="phone-number" class="input-form" placeholder="Phone Number"
                    minlength="10" maxlength="10" v-model="phoneNumber" @input="validatePhoneNumber" />
                <p id="phone-error" v-if="phoneError" style="color: red">
                    หมายเลขโทรศัพท์ต้องมี 10 หลักและเป็นตัวเลขเท่านั้น
                </p>
            </div>
            <p id="form1-error" class="error-message" v-if="form1Error">
                กรุณากรอกข้อมูลให้ครบถ้วน
            </p>
            <button type="button" class="btn" @click="currentForm = 2">ถัดไป</button>
        </div>

        <div id="form2"  v-show="currentForm === 2">
            <div class="formbox">
            ...
            <div id="individual-container" v-show="sellerType === 'individual'">
                <div class="formbox">
                    ...
                    <input type="radio" id="prefix-option1" name="prefix" value="Ms" v-model="prefix"
                        @click="showOtherPrefixInput(false)" />
                    <label for="prefix-option1" class="section">นางสาว</label>
                    <input type="radio" id="prefix-option2" name="prefix" value="Mr" v-model="prefix"
                        @click="showOtherPrefixInput(false)" />
                    <label for="prefix-option2" class="section">นาย</label>
                    <input type="radio" id="prefix-option3" name="prefix" value="Mrs" v-model="prefix"
                        @click="showOtherPrefixInput(false)" />
                    <label for="prefix-option3" class="section">นาง</label>
                    <input type="radio" id="prefix-option4" name="prefix" value="Other" v-model="prefix"
                    ...
                    <div id="other-prefix-container" v-show="prefix === 'Other'">
                        <input type="text" name="other-prefix" id="other-prefix" class="input-form"
                            placeholder="Other..." v-model="otherPrefix" />
                ...
                <div class="formbox">
                    ...
                    <div class="formbox">
                        <textarea name="address-details" id="address-details" class="input-form"
                            placeholder="รายละเอียดที่อยู่ ระบุตามที่อยู่ตามบัตรประชาชน"
                            v-model="addressDetails"></textarea>
                    </div>
                </div>
                <div class="formbox">
            ...
            <div id="corporate-container" v-show="sellerType === 'corporate'">
                ...
                <div class="formbox">
                    ...
                    <input type="text" v-model="corporateRegistrationNumber" class="input-form"
                        placeholder="Corporate Registration Number" minlength="13" maxlength="13"
                        @input="validateCorporateRegistrationNumber" />
                    <p v-show="corporateRegistrationNumberError" style="color: red">
                ...
                <div class="formbox">
                    ...
                    <div class="formbox">
                        <textarea v-model="addressDetails" class="input-form"
                            placeholder="ที่อยู่จดทะเบียนบริษัท กรอกที่อยู่ตามหนังสือรับรอง/ภ.พ.20 ฉบับล่าสุด"></textarea>
                    </div>
                </div>
                <div class="formbox">
                    <label class="section">หนังสือรับรองบริษัท :</label>
                ...
                <p id="form2-error" class="error-message" v-if="form2Error">
                    กรุณากรอกข้อมูลให้ครบถ้วน
                </p>
                
            </div>
            <button type="button" class="btn-back"  @click="currentForm = 1">
                    ย้อนกลับ
                </button>
                <button type="button" class="btn" @click="currentForm = 3">ถัดไป</button>
        </div>

        <div id="form3" v-show="currentForm === 3">
            <div class="formbox">
            ...
            <button type="button" class="btn-back" @click="showForm2">
                ย้อนกลับ
            </button>
            <button type="button" class="btn" @click="currentForm = 4">
                ยืนยัน
            </button>
        </div>

        <div id="form4" v-show="currentForm === 4">
            <div class="formbox">
...
export default {
    ...
    methods: {
        login() {
            axios
                .post("http://localhost:8081/users/selling", {})
                .then((response) => {
    ...
    const idCardNumberInput = this.$refs.idCardNumber;
    const idCardError = this.$refs.idCardError;

    if (idCardNumberInput && idCardError) {
    ...
    return false; // or handle case where inputs are not defined
},
        validateCorporateRegistrationNumber() {
            const corporateRegistrationNumber =
                this.$refs.corporateRegistrationNumber.value;
            const corporateRegistrationError = this.$refs.corporateRegistrationError;

            if (
                corporateRegistrationNumber.trim() !== "" &&
                !isNaN(corporateRegistrationNumber) &&
                corporateRegistrationNumber.length === 13
            ) {
        ...
        showForm4() {
            ...
            this.saveFormDataToFile(formData)
                .then(() => {
                    if (this.validateForm3()) {
                        this.$refs.form2.style.display = "none";
                        this.$refs.form3.style.display = "none";
